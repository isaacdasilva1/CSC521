...
$sql = "SELECT token from ACCOUNT WHERE User_ID ='$username'";
...
if(isset($_POST['submit']) && $tkn == $token && was_thirty_mins_ago($connect, $username))
{
	...
	$sql = "SELECT * From Location WHERE Event_ID = '".$location_id."'";
    ...
    $sql_report = "INSERT INTO Report (Description, Report_ID, User_ID) VALUES ('{$description}','{$report_ID}','{$username}')";
	
    //Variables that make detect if the report went through OK.
    $query1 = false;
    $query2 = false;
	
    if((mysqli_query($connect, $sql_report)))
        $query1 = true;
   
	$sql_time = "INSERT INTO TIME (Event_ID, HOUR, MINUTE, Report_ID, SECOND, Time_Stamp) VALUES ('{$location_id}','{$hours}','{$minutes}', '{$report_ID}', 0, NULL)";
    
    if(mysqli_query($connect, $sql_time))
        $query2 = true;
...
function get_report_id()
{
	...
	$result = mysqli_query($connect, "SELECT * FROM Report WHERE Report_ID = '{$id}'");
...
function was_thirty_mins_ago($connect, $username)
	{
		$sql_timestamp = "SELECT TIME.Time_Stamp, TIME.Report_ID, Report.User_ID FROM Report INNER JOIN TIME ON Report.User_ID = '$username' ORDER BY Time_Stamp DESC";
